import pytest
import pandas as pd
from asf.selectors import PairwiseClassifier, MultiClassClassifier, PairwiseRegressor
...
def test_pairwise_classifier(dummy_performance, dummy_features, dummy_metadata):
    classifier = PairwiseClassifier(model_class=RandomForestClassifier, metadata=dummy_metadata)
...
def test_multi_class_classifier(dummy_performance, dummy_features, dummy_metadata):
    classifier = MultiClassClassifier(model_class=RandomForestClassifier, metadata=dummy_metadata)
    classifier.fit(dummy_features, dummy_performance)
    predictions = classifier.predict(dummy_features)
    assert len(predictions) == 3
...
def test_pairwise_regressor(dummy_performance, dummy_features, dummy_metadata):
    regressor = PairwiseRegressor(model_class=RandomForestRegressor, metadata=dummy_metadata)
